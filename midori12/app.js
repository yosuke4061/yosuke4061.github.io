...
function addMoon() {
    const geometry = new THREE.SphereGeometry(3, 32, 32); // 半径を3に増やす
    const material = new THREE.MeshBasicMaterial({ color: 0xFFD700 }); // 金色のマテリアル
    const moon = new THREE.Mesh(geometry, material);
    moon.position.set(-10, 10, -20); // 位置を調整（シーンの後ろに配置）
    scene.add(moon);
}
function addImageWithEffects() {
    const textureLoader = new THREE.TextureLoader();
    const texture = textureLoader.load('20181110-DSC00675.jpg');
    const material = new THREE.MeshBasicMaterial({ map: texture, side: THREE.DoubleSide }); // 裏側からも見えるように設定
    const geometry = new THREE.PlaneGeometry(5, 3);
    const plane = new THREE.Mesh(geometry, material);
    plane.position.set(0, 0, 0); // 中央に配置
    scene.add(plane);

    // 白色の枠を追加
    const frameGeometry = new THREE.BoxGeometry(5.1, 3.1, 0.1);
    const frameMaterial = new THREE.MeshBasicMaterial({ color: 0xffffff });
    const frame = new THREE.Mesh(frameGeometry, frameMaterial);
    frame.position.set(0, 0, -0.1); // 画像の少し後ろに配置
    scene.add(frame);
}

function addFlatSpiralRotationEffect() {
    let cubes = [];
    let maxTrailCount = 36; // 各立方体のトレイルの最大数
