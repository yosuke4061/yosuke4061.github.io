...
async function loadOBJAnimation() {
    ...
    for (let i = 1; i <= totalFrames; i++) {
        ...
        let objUrl = `${basePath}${objBaseName}${index}.obj`;
        let mtlUrl = `${basePath}${mtlBaseName}${index}.mtl`;

        try {
            let mtl = await new Promise((resolve, reject) => {
                mtlLoader.load(mtlUrl, resolve, (xhr) => {
            ...
            let obj = await new Promise((resolve, reject) => {
                ...
                objLoader.load(objUrl, resolve, (xhr) => {
                    if (xhr.loaded === xhr.total) {
                        loadedFiles++;
                        updateOverallProgress();
                    }
            ...
            console.error(`モデルまたはマテリアルの読み込み中にエラーが発生しました。インデックス: ${index}`, error);
    ...
    async function customizeAndUpdateMaterials(models, materials) {
        models.forEach((model, index) => {
            // モデルの各メッシュを走査し、条件に応じてマテリアルを適用
            model.traverse((child) => {
                if (child.isMesh) {
                    if (child.material.name === "Liquid_Domain_Material") {
                        // 特定のマテリアルにのみカスタムマテリアルを適用
                    ...
                    // 更新されたマテリアルを配列に保存
                    materials[index] = child.material;
                }
            });
        });
    }
    ...
    async function customizeAndUpdateMaterials(models, materials) {
        ...
        models.forEach((model, index) => {
            let waterMaterial = new THREE.MeshPhysicalMaterial({
                ...
                side: THREE.DoubleSide, // 両面をレンダリング
                envMap: scene.environment, // 環境マップを設定
                envMapIntensity: 0.5// 環境マップの強度を調整
            });
