...
function addShader() {
    ...
    bloomPass.strength = 0.5; // ブルームの強度を増加
    bloomPass.radius = 0.55; // ブルームの半径を増加
    ...
    cube.position.set(0, 0, 0); // 立方体の位置を設定
    cube.layers.enable(1); // 立方体をブルームレイヤーに追加
    scene.add(cube); // シーンに立方体を追加
    const cube2 = new THREE.Mesh(cubeGeometry, cubeMaterial);
    cube2.position.set(0, 5, 0); // 立方体の位置を設定
    scene.add(cube2); // シーンに立方体を追加
    const cube3 = new THREE.Mesh(cubeGeometry, cubeMaterial);
    cube3.position.set(0, 0, 5); // 立方体の位置を設定
    scene.add(cube3); // シーンに立方体を追加
}
...
function init() {
    ...
    renderer.toneMappingExposure = 0.12; // 露出を設定
...
function animate() {
    ...
    // ブルームエフェクトが適用されないオブジェクトを暗くする
    scene.traverse(darkenNonBloomed);

    // ブルームエフェクトのレンダリングのために背景を透明に設定
