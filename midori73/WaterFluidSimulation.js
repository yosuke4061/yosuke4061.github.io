...
/**        this.particleSystem = new THREE.Group(); // パーティクルシステムのグループ
        ...
        this.options = {
            ...
            restDensity: 500 // 粒子の理想的な密度 この値は、粒子がどれだけ密集しているか（または密集すべきか）を示します。
        };
        this.gravity = new THREE.Vector3(0, -9.8, 0); // 重力の設定（地球の重力加速度に近似）
        this.bounds = 20; // X軸とZ軸の境界の大きさ
        this.heightBounds = 300; // Y軸の境界の大きさを別に設定
        this.deltaTime = 1 / 30; // フレームレートの逆数（秒単位の時間ステップ）
...
export class WaterFluidSimulation {
    constructor(scene) {
        ...
        this.particles = []; // パーティクルオブジェクトを格納する配列
        this.options = {
            particleCount: 200, // パーティクルの総数
            particleSize: 10, // パーティクルのサイズ
            particleColor: 0x00ff00, // パーティクルの色（緑）
            cohesionDistance: 0.5, // 粒子の体積の0.5倍の距離まで結合できる
            stiffness: 1000, // 圧力の応答の強さを調整する係数 この値が高いほど、粒子間の反発力が強くなります。
    ...
    calculateForces() {
        // 各粒子に対して力を計算し、速度を更新します。
        this.particles.forEach(particle => {
            let pressureForce = new THREE.Vector3(0, 0, 0); // 圧力による力
            let viscosityForce = new THREE.Vector3(0, 0, 0); // 粘性による力

            // 他の全粒子との相互作用を計算
            this.particles.forEach(otherParticle => {
                let r = particle.position.distanceTo(otherParticle.position); // 粒子間の距離
                if (r < this.options.kernelRadius) { // カーネル半径内の場合
                    let grad = this.gradientKernelFunction(r, this.options.kernelRadius); // 勾配カーネル関数を使用
                    // 圧力力の計算
                    ...
                    // 粘性力の計算
                    let velocityDiff = otherParticle.userData.velocity.clone().sub(particle.userData.velocity);
    ...
    gradientKernelFunction(r, h) {
        // 勾配カーネル関数を計算します。この関数は、粒子間の距離とカーネル半径に基づいて勾配を返します。
        if (r < 0 || r > h) return new THREE.Vector3(0, 0, 0); // 範囲外の場合はゼロベクトルを返す

        const factor = -945 / (32 * Math.PI * Math.pow(h, 9));
        const r2 = r * r;
        const h2 = h * h;
        ...
        return direction.multiplyScalar(coefficient);
    }
