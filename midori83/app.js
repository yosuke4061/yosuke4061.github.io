...
let models = [];
let frameCount = 100; // 1500フレームのアニメーションがあるとします
...
function loadOBJAnimation() {
    ...
    let playbackSpeed = 5;
    let forward = true;
    let tessellateModifier = new TessellateModifier(8, 6); // 最大エッジ長と最大分割数を指定
    ...
    function loadModel(index) {
        ...
        let mtlFileName = `01/cc${String(index).padStart(4, '0')}.mtl`;
        let objFileName = `01/cc${String(index).padStart(4, '0')}.obj`;
            ...
            objLoader.load(objFileName, function (object) {
                object.traverse(function (child) {
                    if (child instanceof THREE.Mesh) {
                        ...
                        tessellateModifier.modify(geometry); // TessellateModifierを適用
