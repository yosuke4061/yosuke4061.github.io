...
function setupAndApplyBloomEffect() {
    ...
    bloomPass.threshold = 0.11; // ブルームが適用されるしきい値
    bloomPass.strength = 0.5; // ブルームの強度
...
function addCubes() {
    ...
    const cubeTexture = textureLoader.load('20220922-_MG_0729.jpg');
    ...
    positions.forEach((position, index) => {
        ...
        if (index === 0) { // 最初の立方体のみブルームレイヤーに追加
...
function init() {
    ...
    renderer.toneMapping = THREE.ACESFilmicToneMapping; // トーンマッピングをReinhardに変更
    renderer.toneMappingExposure = 0.5; // 露出を調整
...
function setupScene() {
    ...
    // 環境光の追加
    const ambientLight = new THREE.AmbientLight(SCENE_SETTINGS.ambientLightColor, 1.0); // 環境光を追加し、光の強度を0.5に設定
