...
function createWater(waveType = 'stormy') {
    const textureLoader = new THREE.TextureLoader();
    textureLoader.load('water_1_blue.jpg', function(texture) {
        const textureAspectRatio = texture.image.width / texture.image.height;
        texture.wrapS = texture.wrapT = THREE.RepeatWrapping;
        texture.repeat.set(4 * textureAspectRatio, 4); // テクスチャの繰り返し設定を縦横比に合わせて調整

        const waterGeometry = new THREE.PlaneGeometry(2500, 2500, 256, 256);
        const waterMaterial = new THREE.ShaderMaterial({
            uniforms: {
                time: { value: 0 },
                waterTexture: { value: texture }
            },
            vertexShader: getVertexShader(waveType),
            fragmentShader: `
        ...
        addAnimationCallback(() => {
            waterMaterial.uniforms.time.value += 0.01; // アニメーションの速度を調整
        });
...
function getVertexShader(waveType) {
    switch (waveType) {
        case 'gentle':
            return `
                uniform float time;
                ...
                void main() {
                    vUv = uv;
                    vec3 pos = position;
                    float height = sin(pos.x * 0.05 + time) * cos(pos.y * 0.05 + time) * 5.0;
                    pos.z += height;
        ...
        case 'moderate':
            return `
                ...
                void main() {
                    vUv = uv;
                    vec3 pos = position;
                    float height = sin(pos.x * 0.025 + time) * cos(pos.y * 0.025 + time) * 10.0;
                    pos.z += height;
                    gl_Position = projectionMatrix * modelViewMatrix * vec4(pos, 1.0);
                }
            `;
        case 'stormy':
            return `
                ...
                void main() {
                    vUv = uv;
                    vec3 pos = position;
                    float height = sin(pos.x * 0.015 + time) * cos(pos.y * 0.015 + time) * 20.0;
                    pos.z += height;
                    gl_Position = projectionMatrix * modelViewMatrix * vec4(pos, 1.0);
                }
            `;
...
function startAnimation() {
    // グリッドヘルパーを作成し、シーンに追加します。これにより、座標軸��見やすくなります。
...
function setupScene() {
    ...
    // 環境光の追加
    const ambientLight = new THREE.AmbientLight(SCENE_SETTINGS.ambientLightColor, 2.0); // 環境光を追加し、光の強度を0.5に設定
