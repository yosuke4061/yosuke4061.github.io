...
function loadAndDisplayOBJWithMTL(objFilePath, mtlFilePath) {
    // MTLファイルを読み込む
    mtlLoader.load(mtlFilePath, function (materials) {
        materials.preload(); // マテリアルのプリロード
        objLoader.setMaterials(materials); // OBJローダーにマテリアルを設定

        // OBJファイルを読み込む
        objLoader.load(objFilePath, function (object) {
            object.position.set(0, 10, 10); // オブジェクトの位置を設定
            object.traverse(function (child) {
                if (child instanceof THREE.Mesh) {
                    // 既存のマテリアルをカスタマイズ
                    const material = new THREE.MeshPhongMaterial({
                        color: child.material.color, // 既存の色を保持
                        specular: 0x111111, // 鏡面反射の色
                        shininess: 100, // 光沢
                        transparent: true,
                        opacity: 0.6,
                        depthWrite: false // 水のような透明物体のレンダリングを改善
                    });
                    child.material = material;
                }
            });
            scene.add(object); // シーンにオブジェクトを追加
        }, 
        function (xhr) {
            console.log(`Loading: ${(xhr.loaded / xhr.total * 100).toFixed(2)}%`); // ロード進行状況をログに出力
        },
        function (error) {
            console.error('An error happened during loading the OBJ file: ', error); // エラーが発生した場合の処理
        });
    }, 
    function (error) {
        console.error('An error happened during loading the MTL file: ', error); // MTLファイルの読み込み中にエラーが発生した場合の処理
    });
}
...
function loadOBJAnimation() {
    ...
    function loadModel(index) {
        ...
        let mtlFileName = `02/aa${String(index).padStart(4, '0')}.mtl`;
        let objFileName = `02/aa${String(index).padStart(4, '0')}.obj`;
    ...
    function finalizeLoading() {
        ...
        addAnimationCallback(() => {
            ...
            currentModelIndex = (currentModelIndex + 1) % frameCount;
...
function init() {
    ...
    let asyncInitTasks = [
        ...
        () => new Promise(resolve => {
            loadOBJAnimation(); // OBJアニメーションの読み込みを開始
            resolve();
        })
...
function startAnimation() {
    ...
    loadAndDisplayOBJWithMTL('02/aa0189.obj','02/aa0189.mtl');
