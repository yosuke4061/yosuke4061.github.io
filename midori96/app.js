...
function drawJapaneseRiverLines(xRange = [-200, 200], yRange = 50, numLines = 50) {
    const numPoints = 500;
    const basePoints = [];
    for (let i = 0; i < numPoints; i++) {
        const x = xRange[0] + ((xRange[1] - xRange[0]) * i / (numPoints - 1));
        const y = Math.sin(x / 10) * yRange;
        basePoints.push(new THREE.Vector3(x, y, 0));
    }
    const baseCurve = new THREE.CatmullRomCurve3(basePoints);
    const baseGeometry = new THREE.BufferGeometry().setFromPoints(baseCurve.getPoints(2000));

    const lines = [];
    const totalYRange = yRange * 4; // Yの総範囲を拡大
    const spacingY = totalYRange / numLines; // 線の間隔を調整

    for (let i = 0; i <= numLines; i++) {
        const initialHue = 360 * i / numLines;
        const color = new THREE.Color(`hsl(${initialHue}, 100%, 50%)`);
        ...
        const currentY = -yRange * 2 + i * spacingY; // Y座標を計算（範囲を拡大）
        line.position.y = currentY;
        line.layers.enable(1);
        scene.add(line);
        lines.push({ line, initialHue, previousY: currentY });
    }

    addAnimationCallback(() => {
        const time = Date.now() * 0.0005;
        lines.forEach((entry, index) => {
            const { line, initialHue, previousY } = entry;
            const newY = -yRange * 2 + (index * spacingY) + Math.sin(time + index) * 20; // Y座標の変動幅を増加
            line.position.y += (newY - line.position.y) * 0.1;
            const hue = initialHue + Math.sin(time + index * 0.2) * 30;
...
function init() {
    ...
    renderer.toneMappingExposure = 0.6; // 露出を設定
    ...
    let asyncInitTasks = [
        ...
        () => new Promise(resolve => {
            setupEnvironmentMap('black_back.jpg');   // 初期の環境マップを設定
            resolve();
        }),
        () => new Promise(resolve => {
