...
export class WaterFluidSimulation {
    ...
    initParticles() {
        ...
        const generateParticle = () => {
            ...
            // 緑系の色でランダムに濃淡をつける。色相を少し変更して多様性を持たせる
            const hue = 100 + Math.random() * 40; // 100から140の範囲で色相を変更
            const saturation = 50 + Math.random() * 50; // 彩度は50%から100%の範囲
            const lightness = 30 + Math.random() * 40; // 明度は30%から70%の範囲
            const randomGreen = new THREE.Color(`hsl(${hue}, ${saturation}%, ${lightness}%)`);
            const material = this.shaderMaterial.clone();
            material.uniforms.color.value = randomGreen;
