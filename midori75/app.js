...
function setupImages() {
    ...
    // 画像1のテクスチャをロード
    const texture1 = loader.load('D90A5434.jpg', (texture) => {
        const aspectRatio = texture.image.width / texture.image.height;
        const geometry = new THREE.PlaneGeometry(aspectRatio * 20, 20); // 縦横比を維持
    ...
    // 画像2のテクスチャをロード
    const texture2 = loader.load('DSC08872.jpg', (texture) => {
        const aspectRatio = texture.image.width / texture.image.height;
        const geometry = new THREE.PlaneGeometry(aspectRatio * 20, 20); // 縦横比を維持
        const material = new THREE.MeshBasicMaterial({ map: texture, side: THREE.DoubleSide });
        const mesh = new THREE.Mesh(geometry, material);
        mesh.position.set(0, -100, 0); // 少し前後に位置をずらす
        mesh.lookAt(camera.position);
        scene.add(mesh);
    });
}
    ...
    const sphereMaterial = new THREE.ShaderMaterial({
        ...
        fragmentShader: `
            ...
            void main() {
                ...
                vec3 lightColor = vec3(0.5, 1.0, 0.5);  // 薄緑色に変更
                vec4 innerColor = vec4(lightColor * intensity, 1.0);
                vec4 outerColor = vec4(0.5, 1.0, 0.5, 1.0);  // 薄緑色に変更
