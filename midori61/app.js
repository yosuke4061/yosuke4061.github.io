...
function setupSkyAndGUI() {
    ...
    // 初期プリセットを設定
    Object.assign(sunParams, presets['Default']);
    updateSunPosition();
...
function init() {
    ...
    camera.lookAt(new THREE.Vector3(0, 0, 0)); // カメラの��点を原点に設定

    // レンダラー作成: アンチエイリアスを有効してクオリティを向上
    renderer = new THREE.WebGLRenderer({ antialias: true });
    renderer.setSize(window.innerWidth, window.innerHeight); // レンダラーのサイズをウンドウに合わせる
...
function startAnimation() {
    ...
    // XYZ軸を示す矢印ヘルパーを作成しシーに追加します。これにより、方向が分かりやすくなります。
    ...
    gui.domElement.style.top = '10px'; // 上から15px位置に設定
...
function setupScene() {
    ...
    scene.background = new THREE.Color(SCENE_SETTINGS.backgroundColor); // 背景の設定
    ...
    directionalLight.castShadow = true; // 影の生成を有効する
    ...
    spotLight.shadow.mapSize.height = 2048; // ポットライトシャドウマップの高さを2048に設定
}
...
function setupControls() {
    ...
    controls.dampingFactor = 0.05; // ダン��ングの強度を設定します。
    ...
    if (/Mobi|Android/i.test(navigator.userAgent)) {
        controls.panSpeed = 0.3; // モイルデバイスのパン速度
