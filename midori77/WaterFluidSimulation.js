...
export class WaterFluidSimulation {
        ...
        this.shaderMaterial = new THREE.ShaderMaterial({
            ...
            fragmentShader: `
                ...
                void main() {
                    ...
                    vec3 viewDir = normalize(vViewPosition); // 視線方向
                    vec3 lightDirection = normalize(vec3(0.5, 0.5, 1.0)); // 光源方向
                    ...
                    vec3 refraction = refract(-viewDir, normalizedNormal, 1.0 / 1.5) * refractionStrength;

                    // 色の混合
                    vec3 result = ambient + diffuse + refraction;
                    gl_FragColor = vec4(result, 0.75); // 透明度を設定してステンドグラスのような見た目に
                }
            `,
            uniforms: {
    ...
    initParticles() {
        ...
        const generateParticle = () => {
            ...
            // ランダムな色相でカラフルな色を生成する
            const hue = Math.random() * 360; // 0から360の範囲で色相をランダムに変更
            const saturation = 50 + Math.random() * 50; // 彩度は50%から100%の範囲
            const lightness = 30 + Math.random() * 40; // 明度は30%から70%の範囲
            const randomColor = new THREE.Color(`hsl(${hue}, ${saturation}%, ${lightness}%)`);
            const material = this.shaderMaterial.clone();
            material.uniforms.color.value = randomColor;
