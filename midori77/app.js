...
import { WaterFluidSimulation } from './WaterFluidSimulation.js';
let fluidSimulation;
...
function setupImages() {
    ...
    // 画像1のテクスチャをロード
    const texture1 = loader.load('D90A1638.jpg', (texture) => {
        const aspectRatio = texture.image.width / texture.image.height;
        const geometry = new THREE.PlaneGeometry(aspectRatio * 5, 5); // 縦横比を維持
    ...
    const texture2 = loader.load('D90A1686.jpg', (texture) => {
        const aspectRatio = texture.image.width / texture.image.height;
        const geometry = new THREE.PlaneGeometry(aspectRatio * 5, 5); // 縦横比を維持
        const material = new THREE.MeshBasicMaterial({ map: texture, side: THREE.DoubleSide });
        const mesh = new THREE.Mesh(geometry, material);
    ...
    const texture3 = loader.load('D90A4124-Edit-Edit-2.jpg', (texture) => {
        const aspectRatio = texture.image.width / texture.image.height;
        const geometry = new THREE.PlaneGeometry(aspectRatio * 5, 5); // 縦横比を維持
        const material = new THREE.MeshBasicMaterial({ map: texture, side: THREE.DoubleSide });
        const mesh = new THREE.Mesh(geometry, material);
    ...
    const texture4 = loader.load('D90A5434.jpg', (texture) => {
        const aspectRatio = texture.image.width / texture.image.height;
        const geometry = new THREE.PlaneGeometry(aspectRatio * 5, 5); // 縦横比を維持
        const material = new THREE.MeshBasicMaterial({ map: texture, side: THREE.DoubleSide });
        const mesh = new THREE.Mesh(geometry, material);
    ...
    const texture5 = loader.load('D90A9497.jpg', (texture) => {
        const aspectRatio = texture.image.width / texture.image.height;
        const geometry = new THREE.PlaneGeometry(aspectRatio * 5, 5); // 縦横比を維持
        const material = new THREE.MeshBasicMaterial({ map: texture, side: THREE.DoubleSide });
        const mesh = new THREE.Mesh(geometry, material);
    ...
    const texture = loader.load('DSC08872.jpg', (texture) => {
        const aspectRatio = texture.image.width / texture.image.height;
        const geometry = new THREE.PlaneGeometry(aspectRatio * 5, 5); // 縦横比を維持
        const material = new THREE.MeshBasicMaterial({ map: texture, side: THREE.DoubleSide });
        const mesh = new THREE.Mesh(geometry, material);
...
function setupAquariumSphere() {
    ...
    const sphereMaterial = new THREE.ShaderMaterial({
        ...
        fragmentShader: `
            ...
            uniform float time;  // 時間変数をuniformとして宣言
            ...
            void main() {
                ...
                // 時間に応じて色が変化するように設定
                vec3 lightColor = vec3(sin(time * 0.5) * 0.5 + 0.5, cos(time * 0.5) * 0.5 + 0.5, sin(time * 0.7) * 0.5 + 0.5);
                vec4 innerColor = vec4(lightColor * intensity, 1.0);
                vec4 outerColor = vec4(lightColor, 1.0);  // カラフルな外側の色に変更
        ...
        uniforms: {
            ...
            lightColor: { value: new THREE.Color(0.7, 0.1, 0.7) }  // 初期色を設定
        },
        transparent: true,
        depthTest: true,
        depthWrite: false,
...
function init() {
    ...
    camera.position.set(0, -150,0); // カメラの位置を設定
    ...
    let asyncInitTasks = [
        ...
        () => new Promise(resolve => {
            setupAquariumSphere(); // コントロールの設定
            resolve();
        }),     
    
        () => new Promise(resolve => {
            fluidSimulation = new WaterFluidSimulation(scene); // コントロールの設定
            resolve();
        }),     
    
        () => new Promise(resolve => {
            setupImages(); // コントロールの設定
            resolve();
        }),              
...
function setupControls() {
    ...
    controls.minPolarAngle = Math.PI; // 最小0度
    ...
    // ズームアウトの最大値を設定
    controls.maxDistance = 300;  // この値を適切な最大ズームアウト距離に設定
