...
function setupWall() {
    // 壁のジオメトリを作成
    const wallGeometry = new THREE.PlaneGeometry(1000, 500, 32, 32); // 幅1000、高さ500の平面

    // テクスチャとマップを読み込み
    const textureLoader = new THREE.TextureLoader();
    const wallTexture = textureLoader.load('renga.jpg');
    const wallNormalMap = textureLoader.load('renga_normal.png');
    const wallDisplacementMap = textureLoader.load('renga_displace.png');
    const wallAOMap = textureLoader.load('renga_ao.png');


    // テクスチャの繰り返し設定
    wallTexture.wrapS = wallTexture.wrapT = THREE.RepeatWrapping;
    wallTexture.repeat.set(10, 5); // 横に10回、縦に5回繰り返す
    ...
    const wallMaterial = new THREE.MeshStandardMaterial({
        ...
        displacementScale: 0.2, // ディスプレースメントの影響度
        metalness: 0, // 金属感は0
        roughness: 0.5 // ラフネス
    });
    ...
    wall.position.set(50, 0, 0); // 壁の位置を設定
    wall.receiveShadow = true; // 影の受け取りを有効にする
...
function setupScene() {
    ...
    spotLight.position.set(100, 500, 100); // スポットライトの位置を設定
