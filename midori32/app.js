    ...
    const shaderMaterial = new THREE.ShaderMaterial({
        ...
        fragmentShader: `
            ...
            void main() {
                ...
                vec3 lightEffect = max(vec3(0.0), 1.0 - distance(lightPosition, vPos) / 100.0) * vec3(1.0, 1.0, 0.9);
                gl_FragColor = vec4(refractedColor + reflectedColor + scatteredLight + lightEffect, 0.9);
            }
        `,
        uniforms: {
            time: { value: 0.0 },
...
function setupAquariumLighting() {
    const pointLight = new THREE.PointLight(0xadd8e6, 1.5, 200);
    pointLight.position.set(0, 50, 0);
    pointLight.name = 'dynamicLight';
...
function updateLightPosition() {
    ...
    const z = Math.cos(time) * 30;
    const dynamicLight = scene.getObjectByName('dynamicLight');
    if (dynamicLight) {
        dynamicLight.position.set(x, 50, z);
    }
}
...
function animate() {
    ...
    if (water) {
        ...
        water.material.uniforms.lightPosition.value.copy(scene.getObjectByName('dynamicLight').position);
    }
}
