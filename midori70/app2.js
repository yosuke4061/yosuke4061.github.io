...
function setupAquariumSphere() {
    const sphereGeometry = new THREE.SphereGeometry(600, 128, 128);
    const sphereMaterial = new THREE.ShaderMaterial({
        vertexShader: `
            ...
            void main() {
                ...
                float noiseAmplitude = 50.0;  // 振幅を大きくする
    ...
    loader.load('1.jpg', function(texture) {
        ...
        const material = new THREE.ShaderMaterial({
            uniforms: {
                ...
                murkiness: { value: 0.5 },
                lightPosition: { value: new THREE.Vector3(0, 20, 10) } // 光源の位置
            },
            vertexShader: `
                uniform float time;
                varying vec2 vUv;
            ...
            `,
            fragmentShader: `
                uniform vec3 lightPosition; // 光源の位置
                uniform float time;
                ...
                void main() {
                    ...
                    float scatter = lightIntensity * depthFactor * lightEffect; // 光の散乱を計算
        ...
        waterFolder.add(waterControls, 'waterDepth', 0.1, 5.0, 0.1).name('Water Depth').onChange(value => {
            material.uniforms.waterDepth.value = value;
        });
        waterFolder.add(waterControls, 'lightIntensity', 0.0, 1.0, 0.1).name('Light Intensity').onChange(value => {
            material.uniforms.lightIntensity.value = value;
        });
        waterFolder.add(waterControls, 'murkiness', 0.0, 1.0, 0.1).name('Murkiness').onChange(value => {
            material.uniforms.murkiness.value = value;
        });
