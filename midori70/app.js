...
function addComplexWireframeObject() {
    // イコサヘドロンジオメトリを作成
    const geometry = new THREE.IcosahedronGeometry(15, 5); // 半径20、詳細レベル
    const wireframeMaterial = new THREE.MeshBasicMaterial({
        color: 0xadd8e6, // 薄いブルー
        wireframe: true
    });
    const wireframeMesh = new THREE.Mesh(geometry, wireframeMaterial);
    wireframeMesh.position.set(0, 0, 0); // シーンの中心に配置
...
function setupAquariumSphere() {
    const sphereGeometry = new THREE.SphereGeometry(600, 300, 300);
    const sphereMaterial = new THREE.ShaderMaterial({
        vertexShader: `
            ...
            void main() {
                ...
                float noiseAmplitude = 100.0;  // 振幅を大きくする
        ...
        fragmentShader: `
            ...
            void main() {
                ...
                intensity = pow(intensity, 150.0);  // 光の強度をより強調
                vec3 lightColor = vec3(0.1, 0.5, 0.7);  // 水の色を青緑色に設定
                vec4 innerColor = vec4(lightColor * intensity, 1.0);  // 内側の色
                vec4 outerColor = vec4(0.1, 0.5, 0.7, 1.0);  // 外側の色も水の色に設定
                gl_FragColor = mix(outerColor, innerColor, step(0.0, -vNormal.z));
            }
        `,
        uniforms: {
            time: { value: 0.0 },
            lightIntensity: { value: 0.2 }  // 初期値を設定
...
function addBubbles() {
    const bubbleMaterial = new THREE.ShaderMaterial({
        ...
        fragmentShader: `
            void main() {
                vec3 color = vec3(0.95, 0.98, 1.0);  // 非常に薄いブルー
                float alpha = 0.1;  // 透明度は非常に低く設定
    ...
    for (let i = 0; i < 300; i++) {
        // ランダムなサイズで水泡を生成
        let scale = Math.random() * 0.5 + 0.5;  // 0.5から1.0の範囲でスケール
        const bubbleGeometry = new THREE.SphereGeometry(scale, 16, 16);
        const bubble = new THREE.Mesh(bubbleGeometry, bubbleMaterial);
        bubble.position.x = Math.random() * 200 - 100;
        bubble.position.y = Math.random() * 900 - 300;
        bubble.position.z = Math.random() * 200 - 100;
        bubbles.add(bubble);
    }
    ...
    addAnimationCallback(() => {
        bubbles.children.forEach(bubble => {
            ...
            bubble.position.x += Math.sin(bubble.position.y * 0.05) * 0.5;
            bubble.position.z += Math.cos(bubble.position.y * 0.05) * 0.5;
            if (bubble.position.y > 600) {
                bubble.position.y = -300;
            }
        });
    });
}

function addUnderwaterImage() {
    const loader = new THREE.TextureLoader();
    const images = ['pic/1.jpg', 'pic/2.jpg', 'pic/3.jpg', 'pic/4.jpg', 'pic/5.jpg', 'pic/6.jpg'];
    const radius = 50; // 円の半径
    const angleStep = Math.PI * 2 / images.length; // 画像を配置する角度のステップ

    images.forEach((imagePath, index) => {
        loader.load(imagePath, function(texture) {
            const aspectRatio = texture.image.width / texture.image.height;
            const geometry = new THREE.PlaneGeometry(15 * aspectRatio, 15);
            const material = new THREE.ShaderMaterial({
                uniforms: {
                    time: { value: 0 },
                    map: { value: texture },
                    waterDepth: { value: 1.0 },
                    lightIntensity: { value: 1.0 }, // 明るさを増加
                    murkiness: { value: 0.5 },
                ...
                fragmentShader: `
                    ...
                    void main() {
                        vec2 uv = vUv;
                        uv.x += sin(uv.y * 4.0 + time) * 0.01;
                        uv.y += cos(uv.x * 4.0 + time) * 0.01;
                        vec4 texColor = texture2D(map, uv);

                        vec3 lightDir = normalize(lightPosition - vPos);
                        float lightEffect = max(dot(lightDir, vec3(0, 1, 0)), 0.0);

                        float depthFactor = exp(-waterDepth * uv.y);
                        float scatter = lightIntensity * depthFactor * lightEffect;

                        gl_FragColor = vec4(texColor.rgb * scatter, texColor.a);
                    }
                `,
                transparent: true,
                depthTest: true,
            ...
            mesh.lookAt(new THREE.Vector3(0, 0, 0)); // すべての画像が中心を向くように設定
...
function setupControls() {
    ...
    // ズームアウトの最大値を設定
    controls.maxDistance = 320;  // この値を適切な最大ズームアウト距離に設定
