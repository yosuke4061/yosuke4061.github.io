...
function init() {
    ...
    camera.position.set(0, 50, 100); // カメラの位置を設定
    ...
    let asyncInitTasks = [
        ...
        () => new Promise(resolve => {
            setupEnvironmentMap('');   // 初期の環境マップを設定
            resolve();
        }),
        () => new Promise(resolve => {
            initPostProcessing();
        ...
        () => new Promise(resolve => {
            const geometry = new THREE.BoxGeometry(1, 1, 1); // ジオメトリの定義
            const material = new THREE.MeshBasicMaterial({color: 0x00ff00}); // マテリアルの定義
            const geometry2 = new THREE.BoxGeometry(1, 1, 1); // ジオメトリの定義
            const material2 = new THREE.MeshBasicMaterial({color: 0x00ff00}); // マテリアルの定義       
            let someMesh = new THREE.Mesh(geometry, material);
            let someMesh2 = new THREE.Mesh(geometry2, material2);
            someMesh2.position.x = 20; // X軸に沿って20の位置に設定

            someMesh.layers.set(0);
            someMesh2.layers.set(1);



            scene.add(someMesh); // シーンにメッシュを追加
            scene.add(someMesh2); // シーンにメッシュを追加
            resolve();
        })
...
function animate() {
    ...
    // ブルームエフェクトを適用するレイヤー（レイヤー0）
    camera.layers.set(0);
    composer.render();

    // ブルームエフェクトを適用しないレイヤー（レイヤー1）のレンダリング
    renderer.clearDepth();
    camera.layers.set(1);
    renderer.render(scene, camera);

    // 全てのレイヤーを再びアクティブにする（必要に応じて）
