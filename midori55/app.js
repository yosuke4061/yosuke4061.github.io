...
let lightningInterval = 500; // 稲妻の生成間隔をミリ秒で設定し、頻度を増やす

function createRefinedLightningEffect() {
    function generateRefinedLightning(startX, startY, startZ, endX, endY, endZ, depth) {
        if (depth > 3) return; // 分岐の深さを増やして、より複雑な稲妻を生成

        const points = [];
        const segments = Math.floor(Math.random() * 30 + 20); // 20から50のセグメントでより長い稲妻
        ...
        for (let i = 0; i < segments; i++) {
            x += dx + Math.random() * 40 - 20; // 前回のxから-20から20の範囲で変化
            y += dy + Math.random() * 40 - 20; // 前回のyから-20から20の範囲で変化
            z += dz + Math.random() * 40 - 20; // 前回のzから-20から20の範囲で変化
            points.push(new THREE.Vector3(x, y, z));
        }

        const curve = new THREE.CatmullRomCurve3(points);
        const geometry = new THREE.TubeGeometry(curve, segments, 0.3, 20, false); // 稲妻の太さを増やす
        ...
        // ライトの追加
        const light = new THREE.PointLight(0xffffff, 0.8, 150); // ライトの強度を上げる
        ...
        function fadeOut() {
            ...
            light.intensity = Math.max(0, 0.8 - (count / 100) * 0.8); // ライトの強度を徐々に下げる
    ...
    function repeatRefinedLightning() {
        ...
        if (currentTime - lastLightningTime > lightningInterval) {
            // ランダムな初期終点を生成
            const startX = Math.random() * 800 - 400; // -400から400の範囲で
            const startY = Math.random() * 800 - 400; // -400から400の範囲で
            const startZ = Math.random() * 800 - 200; // -400から400の範囲で
            const endX = Math.random() * 800 - 400;   // -400から400の範囲で
            const endY = Math.random() * 800 - 400;   // -400から400の範囲で
            const endZ = Math.random() * 800 - 400;   // -400から400の範囲で
            generateRefinedLightning(startX, startY, startZ, endX, endY, endZ, 0); // 分岐の深さは0
            lastLightningTime = currentTime;
            lightningInterval = Math.random() * 2000 + 500; // 次の稲妻までの間隔をランダムに設定し、頻度を増やす
        }
    }
