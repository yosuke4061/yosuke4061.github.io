...
async function loadOBJAnimation() {
    const basePath = '01/';
    const objBaseName = 'cc';
    const mtlBaseName = 'cc';
    const totalFrames = 250; // 連番ファイルの総数
    let models = [];
    let materials = [];
    ...
    addAnimationCallback(() => {
        ...
        model.scale.set(3, 4, 6); // スケール調整
        model.position.set(0, -20, 0); // 位置調整
        scene.add(model);
...
function init() {
    ...
    renderer.outputEncoding = THREE.sRGBEncoding; // レンダラーの出力エンコーディングを設定
...
function setupEnvironmentMap() {
    ...
    textureLoader.load('DayEnvironmentHDRI003_4K-TONEMAPPED.jpg', function (texture) {
...
function setupControls() {
    ...
    // ズームアウトの最大値を設定
    controls.maxDistance = 150;  // この値を適切な最大ズームアウト距離に設定
